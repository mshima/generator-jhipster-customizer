  ...
  return class GeneratorExtender extends Superclass {
    constructor(args, options) {
      ...
      if (!customizerBlueprints) {
        ...
        if (options.customizerBlueprints) {
          customizerBlueprints = customizerBlueprints.concat(options.customizerBlueprints.split(','));
        }
      }
      ...
      if (!compatibleGenerator) {
        ...
        debug('Composing');
        info(`Using customizer blueprint ${chalk.yellow(compatibleGenerator.blueprint)} for ${chalk.yellow(generatorName)} subgenerator`);
        this.composeWith(compatibleGenerator, {...options, fromCustomizer: true});
      }

      if (generators.length > 0) {
        generators.forEach(generator => {
          info(`Using customizer blueprint ${chalk.yellow(generator.blueprint)} for ${chalk.yellow(generatorName)} subgenerator`);
          this.composeWith(generator, {...options, fromCustomizer: true});
        });
      }
    }
  };
