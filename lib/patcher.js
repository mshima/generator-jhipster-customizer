...
module.exports = class Patcher {
    ...
    applyDirs(allParsed) {
        this.load(allParsed, 'dir').forEach(dir => {
    ...
    writeFiles(allParsed) {
        ...
        this.load(allParsed, 'files').forEach(fileTemplate => {
    ...
    processPartialTemplates(parsed) {
        ...
        let allSuccess = true;
        this.load(parsed, 'patch').forEach(templates => {
    ...
    load(allParsed, type) {
        ...
        return allParsed.filter(p => p.type === type).map(loadTemplate);
    }
};
