...
const verify = function(generator, obj) {
    ...
    if (obj.dir !== undefined) {
        const notVerified = obj.dir.find(dir => {
...
module.exports = class Patcher {
    ...
    patch(options = {}) {
        ...
        if (Array.isArray(options)) {
            options.forEach(opts => {
                allParsed = allParsed.concat(this.lookForPatches(opts));
            });
        } else {
     ...
     */
    applyDirs(allParsed) {
        allParsed.filter(p => p.type === 'dir').map(loadTemplate).forEach(dir => {
    ...
    writeFiles(allParsed) {
        ...
        allParsed.filter(p => p.type === 'files').map(loadTemplate).forEach(fileTemplate => {
    ...
    processPartialTemplates(parsed) {
        ...
        let allSuccess = true;
        parsed.filter(p => p.type === 'patch').map(loadTemplate).forEach(templates => {
            ...
            templates.tmpls.forEach((item, index) => {
                ...
                files.forEach(file => {
                    ...
                    if (item.type === 'replaceContent') {
                        ...
                        success = needleFile.addContent(target, tmpl);
                    }
                    ...
                    if (success === false || item.debug) {
                        try {
                            ...
                            trace(`Target: ${target}`);
                            trace(body);
                            let debugCb = msg => generator.log.error(msg);
                            if (debug.enabled) {
                                debugCb = debug;
                            }
    ...
    filter(templates, rootPath) {
        ...
        const parsed = templates.map(file => {
            ...
            const ret = { origin: template, feature, filename, type: 'patch' };
            if (sfilename === 'files') {
                ret.type = 'files';
            } else if (sfilename === 'index') {
                ret.type = 'dir';
                ret.dirname = `${path.dirname(file)}${path.sep}`;
            }
