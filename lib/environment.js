...
const createJHipsterGenerator = (generator, env, options) => {
  return legacyCreateGenerator(generator, options, env.getPackagePath('jhipster:app'));
};

const legacyCreateGenerator = function (generator, options = {}, packagePath = Env.lookupGenerator('jhipster:app', {packagePath: true})) {
  // Fconst generator = env.requireGenerator(`jhipster:${generator}`);
  ...
  // Create the class.
  const extended = improver(parent, {path: paths, version: jhipsterVersion});
